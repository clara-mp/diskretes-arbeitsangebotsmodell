# Read in data file
data_nv_f <- read.csv2("data_hm_nv_f.csv")
...
### Set core controls
apollo_control = list(
  modelName  = "clogit_nv_f",
  modelDescr = "Conditonal logit model - female, not married",
  indivID    ="pid",
  mixing     = FALSE,
  HB= FALSE,
  nCores     = 1, 
...
clogit_nv_f = apollo_estimate(apollo_beta, apollo_fixed,
...
#### MODEL OUTPUTS                                               ##
# ################################################################# #
apollo_saveOutput(clogit_nv_f)

summary(clogit_nv_f)

### Use the estimated model to make predictions
predictions_base = apollo_prediction(clogit_nv_f, apollo_probabilities, apollo_inputs,
...
apollo_inputs = apollo_validateInputs()
predictions_new = apollo_prediction(clogit_nv_f, apollo_probabilities, apollo_inputs)  
...
## ELasticity
elasticity_nv_f <- ((worktime_new/worktime_base)-1) /1.1
